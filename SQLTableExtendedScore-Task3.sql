...
SELECT 
	...
	total_employed, 
	toilet_total,
	(COALESCE(stops_total, 0) + COALESCE(retail_total_businesses, 0) + COALESCE(health_total_businesses, 0) + COALESCE(polling_total, 0) + COALESCE(primary_catchments, 0) + COALESCE(secondary_catchments, 0) + COALESCE(future_catchments, 0)+ COALESCE(traffic_camera_count, 0) + COALESCE(total_employed, 0) + COALESCE(toilet_total, 0)) AS x_val
FROM sa2_boundaries
...
LEFT JOIN filtered_traffic
ON sa2_boundaries.sa2_code21 = filtered_traffic.sa2_code21
LEFT JOIN filtered_unemployment
ON sa2_boundaries.sa2_code21 = filtered_unemployment.sa2_code
LEFT JOIN filtered_toilets
ON sa2_boundaries.sa2_code21 = filtered_toilets.sa2_code21
